...
function create_config_file(config_path) {
    ...
    if (config_path.type === 'xdg') {
        var data_dir = process.env.XDG_DATA_HOME || Path.join(process.env.HOME, '.local', 'share');
...
function get_paths() {
    var try_paths = [];
    var xdg_config = process.env.XDG_CONFIG_HOME || process.env.HOME && Path.join(process.env.HOME, '.config');
