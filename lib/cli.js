...
if (process.getuid && process.getuid() === 0) {
    global.console.error('nexxtopia doesn\'t need superuser privileges. Don\'t run it under root.');
}

process.title = 'nexxtopia';
require('es6-shim');
...
if (commander.args.length == 1 && !commander.config) {
    // handling "nexxtopia [config]" case if "-c" is missing in commandline
...
function afterConfigLoad() {
    ...
    get_listen_addresses().forEach(function(addr) {
        ...
        if (addr.proto === 'https') {
            ...
            if (!config.https || !config.https.key || !config.https.cert) {
                ...
                logger.logger.fatal([
                    ...
                    ' $ openssl genrsa -out ' + conf_path('nexxtopia-key.pem') + ' 2048',
                    ' $ openssl req -new -sha256 -key ' + conf_path('nexxtopia-key.pem') + ' -out ' + conf_path('nexxtopia-csr.pem'),
                    ' $ openssl x509 -req -in ' + conf_path('nexxtopia-csr.pem') + ' -signkey ' + conf_path('nexxtopia-key.pem') + ' -out ' + conf_path('nexxtopia-cert.pem'),
                    '',
                    'And then add to config file (' + conf_path() + '):',
                    '  https:',
                    '    key: nexxtopia-key.pem',
                    '    cert: nexxtopia-cert.pem'
                ].join('\n'));
                process.exit(2);
            }
        ...
        logger.logger.warn({
            ...
            version: 'nexxtopia/' + pkg.version
        }, 'http address - @{addr}');
    });
    // undocumented stuff for tests
    if (typeof process.send === 'function') {
        process.send({
            nexxtopia_started: true
        });
    }
}
