...
module.exports = function(config, auth, storage) {
    ...
    function isLoggedIn(req, res, next) {
        ...
        res.redirect(config.web.login_redirect_url);
    }
    ...
    app.get('/home', isLoggedIn, function(req, res, next) {
        var base = config.url_prefix ? config.url_prefix.replace(/\/$/, '') : req.protocol + '://' + req.get('host');
    ...
    app.get('/logout', function(req, res) {
        req.logout();
        res.redirect(config.web.logout_redirect_url || '/');
    });
    ...
    app.get('/-/logo', function(req, res, next) {
        res.sendFile(config.web && config.web.logo ? config.web.logo : __dirname + '/static/logo-sm.png');
    });
