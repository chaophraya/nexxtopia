...
function Config(config) {
    ...
    for (var i in self.users) {
        ...
        assert(
            ...
            self.users[i].password.match(/^[a-f0-9]{40}$/), 'CONFIG: wrong password format for user: ' + i + ', sha1 expected')
    }
    ...
    for (var i in self.packages) {
        assert(
            ...
            !Array.isArray(self.packages[i]), 'CONFIG: bad "' + i + '" package description (object expected)')
