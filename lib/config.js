...
function Config(config) {
    ...
    if (!self.user_agent)
        self.user_agent = 'Sinopia/' + pkg.version;
    ...
    self.localList = LocalData(Path.join(Path.resolve(Path.dirname(self.self_path), self.storage), '.sinopia-db.json'));
    if (!self.secret) {
    ...
    for (var i in self.users) {
        ...
        assert(typeof self.users[i].password === 'string' && self.users[i].password.match(/^[a-f0-9]{40}$/), 'CONFIG: wrong password format for user: ' + i + ', sha1 expected');
    }

    for (var i in self.uplinks) {
        assert(self.uplinks[i].url, 'CONFIG: no url for uplink: ' + i);
        assert(typeof self.uplinks[i].url === 'string', 'CONFIG: wrong url format for uplink: ' + i);
    ...
    for (var i in self.packages) {
        assert(typeof self.packages[i] === 'object' && !Array.isArray(self.packages[i]), 'CONFIG: bad "' + i + '" package description (object expected)');
        self.packages[i].access = normalize_userlist(self.packages[i].allow_access, self.packages[i].access);
        delete self.packages[i].allow_access;
        self.packages[i].publish = normalize_userlist(self.packages[i].allow_publish, self.packages[i].publish);
        delete self.packages[i].allow_publish;
        self.packages[i].proxy = normalize_userlist(self.packages[i].proxy_access, self.packages[i].proxy);
    ...
    if (self.ignore_latest_tag == null)
        self.ignore_latest_tag = false;
    return self;
}
...
module.exports.parse_interval = function(interval) {
    ...
    interval.split(/\s+/).forEach(function(x) {
        ...
        if (!m || parse_interval_table[m[4]] >= last_suffix || m[4] === '' && last_suffix !== Infinity) {
