...
function Auth(config) {
    ...
    self.plugins.unshift({
        ...
        authenticate: function(user, password, cb) {
            if (config.users != null && config.users[user] != null && Crypto.createHash('sha1').update(password).digest('hex') === config.users[user].password) {
                return cb(null, [user]);
            }
            return cb();
        },
